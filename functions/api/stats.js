...
export async function onRequestGet(context) {
  ...
  async function sql(query) {
    ...
    if (!contentType.includes('json')) {
      // 典型错误：SQL 语法问题。响应是 plain text 如 "Input was invalid: sql parser error: ..."
      const compactQuery = query.replace(/\s+/g, ' ').slice(0, 150);
      throw new Error(`SQL 语法错误 [${status}]: ${text.slice(0, 200)} | query: ${compactQuery}`);
    }
    let j;
    try { j = JSON.parse(text); }
    catch (e) { throw new Error(`SQL 响应 JSON 解析失败 [${status}]: ${text.slice(0, 200)}`); }
    if (j.errors && j.errors.length > 0) {
      throw new Error('SQL failed: ' + JSON.stringify(j.errors));
    }
    if (!Array.isArray(j.data)) {
      throw new Error('SQL 响应无 data 字段: ' + text.slice(0, 200));
    }
    return j.data;
  }
